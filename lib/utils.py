...
def merge_ivs(a, b):
    if a.chrom != b.chrom:
        print >>sys.sdterr, "merge_ivs: mismatching chromosomes!"
        sys.exit(1)
    if a.strand != b.strand:
        print >>sys.sdterr, "merge_ivs: mismatching strands!"
        sys.exit(1)
    start   = min(a.start, b.start)
    end     = max(a.end, b.end)
    c       = ht.GenomicInterval(a.chrom, start, end, a.strand)
    return c
